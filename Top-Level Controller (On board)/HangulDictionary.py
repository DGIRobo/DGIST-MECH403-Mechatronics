...
class Hangul:
    def __init__(self):
        ...
        self.sampling = list(range(0, 361, 20))
        ...
        self.wa = self.offset(self.resize(self.o, 4/5, 1/5), 0, 6/10) + self.offset(self.resize(self.a, 1/5, 1), 4/5, 0)
        self.wae = self.offset(self.resize(self.o, 4/5, 1/5), 0, 6/10) + self.offset(self.resize(self.ae, 1/5, 1), 4/5, 0)
        ...
        self.wo = self.offset(self.resize(self.u, 4/5, 3/10), 0, 7/10) + [(1/2, 17/20, False), (1/2, 17/20, True), (4/5, 17/20, True), (4/5, 17/20, False), (4/5, 0, False), (4/5, 0, True), (4/5, 1, True), (4/5, 1, False)]
        self.we = self.offset(self.resize(self.u, 4/5, 3/10), 0, 7/10) + [(1/2, 17/20, False), (1/2, 17/20, True), (4/5, 17/20, True), (4/5, 17/20, False), (4/5, 0, False), (4/5, 0, True), (4/5, 1, True), (4/5, 1, False), (1, 0, False), (1, 0, True), (1, 1, True), (1, 1, False)]
        ...
        self.oe = self.offset(self.resize(self.o, 4/5, 1/5), 0, 6/10) + self.offset(self.resize(self.i, 1/5, 1), 7/10, 0)
        self.wi = self.offset(self.resize(self.u, 4/5, 3/10), 0, 7/10) + self.offset(self.resize(self.i, 1/5, 1), 7/10, 0)
        self.ui = self.offset(self.resize(self.eu, 4/5, 3/10), 0, 6/10) + self.offset(self.resize(self.i, 1/5, 1), 7/10, 0)
    ...
    def string_dot(self, size, chars):
        ...
        for char in chars:
            if char != 'space':
                if i+size > 165:
                    yoffset = yoffset + 1.5*size
                    i = 0
                char = self.offset(char, i, yoffset)
                string_dot_list = string_dot_list + char
                i = i + 1.1*size
            else:
                i = i + 1.1*size
        string_dot_list = string_dot_list + [(0, yoffset + 1.5*size, False)]
    ...
    def Type1(self, size, word1, word2):
        ...
        return self.resize(word1+word2, 1, 1)

    def Type2(self, size, word1, word2, word3):
        temp = self.Type1(1, word1, word2)
        temp = self.resize(temp, size, size*55/100)
        word3 = self.resize(word3, size*4/5, size*35/100)
        word3 = self.offset(word3, size*1/10, size*65/100)
    ...
    def Type4(self, size, word1, word2, word3):
        temp = self.Type3(1, word1, word2)
        temp = self.resize(temp, size, size*65/100)
        word3 = self.resize(word3, size*4/5, size*25/100)
        word3 = self.offset(word3, size*1/10, size*75/100)
        return self.resize(temp+word3, 1, 1)

    def Type5(self, size, word1, word2):
        word1 = self.resize(word1, size*0.6, size*0.5)
        word1 = self.offset(word1, size*0.1, size*0.1)
        word2 = self.resize(word2, size, size)
        return self.resize(word1+word2, 1, 1)

    def Type6(self, size, word1, word2, word3):
        temp = self.Type5(1, word1, word2)
        temp = self.resize(temp, size, size*65/100)
        word3 = self.resize(word3, size*7/10, size*25/100)
        word3 = self.offset(word3, size*1/10, size*75/100)
        return self.resize(temp+word3, 1, 1)
    ...
    def TrajectoryMaker(self, size, dividedHangul):
        ...
        for word in dividedHangul:
            if word[0] == 'Space':
                Hangul.TrajectoryList.append('space')
            if word[0] == 'Type1':
...
if __name__ == '__main__':
    H = Hangul()
    # Combine
    H.stringShower(1, 3, H.string_dot(1, [H.Type2(1, H.k, H.eo, H.m), H.Type3(1, H.p, H.yu), H.Type1(1, H.t, H.eo)]))
    H.stringShower(1, 2, H.string_dot(1, [H.Type2(1, H.j, H.eo, H.r), H.Type1(1, H.d, H.ae)]))
    H.stringShower(1, 2, H.string_dot(1, [H.Type1(1, H.ng, H.a), H.Type1(1, H.h, H.a)]))
    H.stringShower(1, 4, H.string_dot(1, [H.Type1(1, H.d, H.i), H.Type1(1, H.j, H.i), H.Type3(1, H.s, H.eu), H.Type3(1, H.t, H.eu)]))
    H.stringShower(1, 4, H.string_dot(1, [H.Type3(1, H.s, H.u), H.Type4(1, H.n, H.eu, H.ng), H.Type4(1, H.t, H.eu, H.g), H.Type2(1, H.g, H.a, H.ng)]))
    H.stringShower(1, len([['Type2', 'ㅂ', 'ㅐ', 'ㄱ'], ['Type2', 'ㅁ', 'ㅣ', 'ㄴ'], ['Type2', 'ㅎ', 'ㅕ', 'ㄱ']]), H.TrajectoryMaker(1, [['Type2', 'ㅂ', 'ㅐ', 'ㄱ'], ['Type2', 'ㅁ', 'ㅣ', 'ㄴ'], ['Type2', 'ㅎ', 'ㅕ', 'ㄱ']]))